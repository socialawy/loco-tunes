...
import { saveProject, loadProject, deleteProject, getProjects, getMotifs, saveMotif as saveMotifToDB, deleteMotif as deleteMotifFromDB } from '@/lib/storage';
import type { Project, SavedMotif } from '@/types/music';
...
interface MusicStore {
  ...
  saveCurrentMelodyAsMotif: (name?: string) => Promise<void>;
...
export const useMusicStore = create<MusicStore>((set, get) => ({
  ...
  fetchMotifs: async () => {
    try {
      const savedMotifs = await getMotifs();
      set({ savedMotifs });
    } catch (err) {
  ...
  saveCurrentMelodyAsMotif: async (name?: string) => {
    const { currentTrack } = get();
    if (!currentTrack) return;

    const melodyStem = currentTrack.stems.find(s => s.type === 'melody');
    if (!melodyStem || melodyStem.notes.length === 0) {
      toast.error('No melody found to save');
      return;
    }

    // Normalize notes to start at 0
    const firstNoteStart = melodyStem.notes[0].startTime;
    const normalizedNotes = melodyStem.notes.map(n => ({
      ...n,
      startTime: n.startTime - firstNoteStart
    }));

    const motif: SavedMotif = {
      id: crypto.randomUUID(),
      name: name || `Melody Motif - ${new Date().toLocaleTimeString()}`,
