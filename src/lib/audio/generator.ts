...
export async function generateTrack(params: GenerationParams): Promise<Track> {
  ...
  let motifToUse: Motif | undefined;
  if (savedMotifId) {
    const motifs = await getMotifs();
    motifToUse = motifs.find(m => m.id === savedMotifId);
  }
  ...
  for (const section of sections) {
    ...
    // Melody
    const sectionMelody = generateMelodyNotes(rootMidi, scale, genre, mood, bpm, section.numBars, complexity, sectionRoots, section.type, motifToUse);
  ...
  for (const section of sections) {
    ...
    switch (stemType) {
      ...
      case 'bass':
        ...
        break;
      case 'melody':
        sectionNotes = generateMelodyNotes(rootMidi, scale, genre, mood, bpm, section.numBars, complexity, sectionRoots, section.type, motifToUse)
          .map(n => ({ ...n, startTime: n.startTime + sectionStartTime }));
        break;
      case 'harmony':
...
export async function generateStemVariation(
  ...
  switch (stem.type) {
    ...
    case 'bass':
      ...
      break;
    case 'melody':
      notes = generateMelodyNotes(rootMidi, scale, genre, mood, bpm, numBars, variedComplexity, chordRoots, sectionType, motifToUse);
      break;
    case 'harmony':
