...
export async function generateTrack(params: GenerationParams): Promise<Track> {
  ...
  for (const section of sections) {
    ...
    // Melody
    const sectionMelody = generateMelodyNotes(rootMidi, scale, genre, mood, bpm, section.numBars, complexity, sectionRoots, section.type, params.seedMotif);
  ...
  for (const section of sections) {
    ...
    switch (stemType) {
      ...
      case 'bass':
        ...
        break;
      case 'melody':
        sectionNotes = generateMelodyNotes(rootMidi, scale, genre, mood, bpm, section.numBars, complexity, sectionRoots, section.type, track.params.seedMotif)
          .map(n => ({ ...n, startTime: n.startTime + sectionStartTime }));
        break;
      case 'harmony':
...
export async function generateStemVariation(
  ...
  switch (stem.type) {
    ...
    case 'bass':
      ...
      break;
    case 'melody':
      notes = generateMelodyNotes(rootMidi, scale, genre, mood, bpm, numBars, variedComplexity, chordRoots, sectionType, params.seedMotif);
      break;
    case 'harmony':
