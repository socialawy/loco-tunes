...
export async function saveMotifToStorage(motif: Motif): Promise<void> {
  await set(`${MOTIF_PREFIX}${motif.id}`, motif);
}

export async function getMotifsFromStorage(): Promise<Motif[]> {
  const allKeys = await keys();
  ...
  // Sort by id descending as a basic way to show newest first, assuming UUIDv4 or timestamp
  return motifs.sort((a, b) => b.id.localeCompare(a.id));
}

export async function deleteMotifFromStorage(id: string): Promise<void> {
