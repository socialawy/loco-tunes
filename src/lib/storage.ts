...
export async function saveMotifStorage(motif: Motif): Promise<void> {
  await set(`${MOTIF_PREFIX}${motif.id}`, motif);
}

export async function loadMotifsStorage(): Promise<Motif[]> {
  const allKeys = await keys();
...
export async function deleteMotifStorage(id: string): Promise<void> {
